...
import { PlanCard } from "../../UI/Main/PlanCard";
...
export const Billing = () => {
  ...
  return (
    <div
      className={`text-black w-full h-full px-[40px] flex items-center flex-col max-md:px-[20px] ${
        theme === "dark" ? "bg-[#1B1F27]" : ""
      }`}
    >
      <BucketHeader />
      {/*       <div className="flex flex-col w-full justify-start mb-[15px]">
        <h2 className="text-poppins text-[24px] font-[600] mb-[32px]">
          Billing
        </h2>
        <p className="text-[20px] leading-[23px] font-[400] text-left">
          Current Plan
        </p>
      </div>
      <div className="flex w-full mb-[75px]">
        {selectPlanArr.map((plan, index) => {
          if (index === 1) {
            return <SelectPlan {...plan} key={plan.id} isBilling={true} />;
          } else {
            return null;
          }
        })}
      ...
      {windowSize >= 1100 && (
        <div className="flex justify-between w-full max-w-[760px] max-md:w-full">
          {planCardsArr.map((card) => (
            <PlanCard {...card} key={nanoid()} isBilling={true} />
          ))}
        </div>
      )}
      {windowSize < 1100 && (
        <div
          ...
          className="flex justify-between w-full max-md:justify-center"
        >
          <button
            onClick={goToPrevSlide}
            className="w-[100px] flex justify-center items-center max-md:hidden"
          >
            {buttonLeft}
          </button>
          {planCardsArr.slice(slide, slide + 1).map((card) => (
            <PlanCard {...card} key={nanoid()} isBilling={true} />
          ))}
          <button
      ...
      <Link to="https://billing.stripe.com/p/login/3csbIK50race3yoeUU">
        <span className={`${theme === "dark" ? "text-[#92929D]" : ""}`}>
          To choose your billing. Click this link
        </span>
      </Link>
    </div>
  );
